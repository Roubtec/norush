...
describe('NoopTelemetry', () => {
  it('implements TelemetryHook without throwing', () => {
    ...
    expect(() => telemetry.histogram('test', 42, { env: 'test' })).not.toThrow();
...
describe('ConsoleTelemetry', () => {
  it('logs counter with [norush] prefix', () => {
    ...
    expect(spy).toHaveBeenCalledWith('[norush] counter requests_queued=5');
    spy.mockRestore();
  });
  ...
  it('logs histogram with [norush] prefix', () => {
    ...
    expect(spy).toHaveBeenCalledWith('[norush] histogram batch_turnaround_ms=12345');
    spy.mockRestore();
  });
  ...
  it('logs event with [norush] prefix', () => {
    ...
    expect(spy).toHaveBeenCalledWith('[norush] event circuit_breaker_tripped');
    spy.mockRestore();
  });
