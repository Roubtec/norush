...
function newResult(requestId: string, batchId: string, overrides?: Partial<NewResult>): NewResult {
  return {
    requestId,
    batchId,
  ...
  describe('Store contract', () => {
    describe('Request lifecycle', () => {
      ...
      test('updateRequest modifies fields', async () => {
        ...
        expect(updated?.updatedAt.getTime()).toBeGreaterThanOrEqual(req.updatedAt.getTime());
      });
    ...
    describe('Result lifecycle', () => {
      test('createResult returns a record with generated ID and defaults', async () => {
        ...
        const result = await store.createResult(newResult(req.id, batch.id));
      ...
      test('getUndeliveredResults returns pending and failed results', async () => {
        ...
        const res2 = await store.createResult(newResult(req2.id, batch.id));
        const res3 = await store.createResult(newResult(req3.id, batch.id));
    ...
    describe('consumePeriodRequests (atomic rate limit)', () => {
      ...
      test('concurrent consume calls never exceed the limit', async () => {
        ...
        const results = await Promise.all(
          Array.from({ length: 10 }, () => store.consumePeriodRequests('test-user', 1, 5)),
        );
    ...
    describe('incrementPeriodTokens', () => {
      ...
      test('does not throw for non-existent user', async () => {
        store = await factory();
        await expect(store.incrementPeriodTokens('nonexistent', 100)).resolves.toBeUndefined();
      });
    });
    ...
    describe('incrementSpend', () => {
      ...
      test('accumulates across multiple calls', async () => {
        ...
        await store.incrementSpend('test-user', 0.5);

        const limits = await store.getUserLimits('test-user');
        if (!limits) throw new Error('expected user limits to exist');
        expect(limits.currentSpendUsd).toBeCloseTo(2.5);
      });

      test('does not throw for non-existent user', async () => {
        store = await factory();
        await expect(store.incrementSpend('nonexistent', 5.0)).resolves.toBeUndefined();
      });
    });
    ...
    describe('Analytics', () => {
      ...
      test('getDetailedStats returns cost breakdown grouped by provider and model', async () => {
        ...
        const claudeEntry = stats.costBreakdown.find((e) => e.provider === 'claude');
        ...
        expect(claudeEntry.standardCostUsd).toBeGreaterThan(claudeEntry.batchCostUsd);

        const openaiEntry = stats.costBreakdown.find((e) => e.provider === 'openai');
