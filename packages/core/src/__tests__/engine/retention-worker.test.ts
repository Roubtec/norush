...
async function createCompletedRequest(
  ...
  if (overrides.createdAt) {
    ...
    const reqRecord = (store as unknown as { requests: Map<string, Request> }).requests.get(
      request.id,
    );
    if (reqRecord) {
    ...
    const resRecord = (store as unknown as { results: Map<string, Result> }).results.get(result.id);
    if (resRecord) {
  ...
  return {
    request: updatedReq,
    result: (await store.getUndeliveredResults(100)).find((r) => r.id === result.id) ?? result,
  };
}
...
describe('RetentionWorker', () => {
  ...
  describe('hard cap enforcement', () => {
    ...
    it('uses default hard cap of 90 days', async () => {
      const now = new Date();
      const oldDate = new Date(now.getTime() - (DEFAULT_HARD_CAP_DAYS + 1) * MS_PER_DAY);
  ...
  describe('tombstone format', () => {
    ...
    it('preserves token counts after scrubbing', async () => {
      ...
      const results = (store as unknown as { results: Map<string, Result> }).results;
  ...
  describe('event log scrubbing', () => {
    it('scrubs event log details for scrubbed request entities', async () => {
      ...
      const requestEvent = events.find((e) => e.entityId === request.id && e.event === 'submitted');
  ...
  describe('error handling', () => {
    ...
    it('handles hard cap errors gracefully', async () => {
      ...
      const brokenStore = {
        ...
        getDistinctUserIdsWithUnscrubbedContent: vi.fn().mockResolvedValue([]),
  ...
  describe('non-terminal requests', () => {
    it('does not scrub queued requests', async () => {
      ...
      const request = await store.createRequest(makeNewRequest({ userId: 'user_queued' }));
    ...
    it('does not scrub in-progress requests', async () => {
      ...
      const request = await store.createRequest(makeNewRequest({ userId: 'user_processing' }));
  ...
  describe('telemetry', () => {
    ...
    it('does not emit sweep_complete when nothing was scrubbed', async () => {
      ...
      expect(eventSpy).not.toHaveBeenCalledWith('retention_sweep_complete', expect.anything());
    });
  });
