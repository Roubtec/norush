...
describe('ClaudeAdapter', () => {
  ...
  describe('submitBatch', () => {
    ...
    it('throws when messages is missing from params', async () => {
      ...
      await expect(adapter.submitBatch([req])).rejects.toThrow(/must include a "messages" array/);
    ...
    it('throws when max_tokens is present but not a number', async () => {
      ...
      await expect(adapter.submitBatch([req])).rejects.toThrow(/invalid "max_tokens"/);
      expect(mockBatchesCreate).not.toHaveBeenCalled();
    });

    it('propagates SDK errors', async () => {
      mockBatchesCreate.mockRejectedValue(new Error('API rate limit exceeded'));

      await expect(adapter.submitBatch([makeRequest()])).rejects.toThrow('API rate limit exceeded');
    });
  });
  ...
  describe('cancelBatch', () => {
    it('calls cancel with the correct batch ID', async () => {
      ...
      await adapter.cancelBatch(makeRef({ providerBatchId: 'msgbatch_cancel_01' }));
