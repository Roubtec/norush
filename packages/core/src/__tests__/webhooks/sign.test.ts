...
describe("verifyWebhookSignature", () => {
  ...
  it("returns false for a wrong secret", () => {
    const signature = signWebhookPayload(secret, body);
    expect(verifyWebhookSignature("wrong-secret", body, signature)).toBe(
      false,
    );
  });

  it("returns false for a completely invalid signature", () => {
    expect(verifyWebhookSignature(secret, body, "not-a-hex-string")).toBe(
      false,
    );
  });
  ...
  it("returns false for a truncated signature", () => {
    const signature = signWebhookPayload(secret, body);
    expect(verifyWebhookSignature(secret, body, signature.slice(0, 32))).toBe(
      false,
    );
  });
