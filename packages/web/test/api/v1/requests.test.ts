...
describe('GET /api/v1/requests', () => {
  ...
  it('rejects invalid status filter', async () => {
    const event = makeEvent(
      ...
      undefined,
      'Bearer valid_token',
      new URLSearchParams({ status: 'invalid' }),
    );
    const response = await GET(event as never);
    expect(response.status).toBe(400);
    const data = await response.json();
  ...
  it('rejects invalid provider filter', async () => {
    const event = makeEvent(
      ...
      undefined,
      'Bearer valid_token',
      new URLSearchParams({ provider: 'google' }),
    );
    const response = await GET(event as never);
    expect(response.status).toBe(400);
    const data = await response.json();
  ...
  it('accepts valid status filter', async () => {
    ...
    const event = makeEvent(
      ...
      undefined,
      'Bearer valid_token',
      new URLSearchParams({ status: 'queued' }),
    );
    const response = await GET(event as never);
    expect(response.status).toBe(200);
  });
  ...
  it('accepts valid provider filter', async () => {
    ...
    const event = makeEvent(
      ...
      undefined,
      'Bearer valid_token',
      new URLSearchParams({ provider: 'claude' }),
    );
    const response = await GET(event as never);
    expect(response.status).toBe(200);
  });
...
describe('GET /api/v1/requests/:id', () => {
  ...
  it('returns request with result when result exists', async () => {
    ...
    expect(data.request.result.response).toEqual({
